...
from gamma_world._src.imaginaire.utils import distributed
from gamma_world._src.imaginaire.utils.misc import Color
...
class JobConfig:
    ...
    def path_local(self) -> str:
        local_root = os.environ.get("IMAGINAIRE_OUTPUT_ROOT", "imaginaire4-output")
...
class StragglerDetectionConfig:
    """Config for Straggler detection tool: https://redacted/dl/gwe/fault_tolerance_related/straggler/-/tree/cupti?ref_type=heads"""
...
class TrainerConfig:
    if TRAINING:
        from gamma_world._src.imaginaire.trainer import ImaginaireTrainer
        from gamma_world._src.imaginaire.utils import callback
...
def load_config(config_path: str, opts: list[str], enable_one_logger: bool = False) -> Config:
    from gamma_world._src.imaginaire.serialization import from_yaml, load_callable
    ...
    if config_path.endswith(".yaml"):
        ...
        from gamma_world._src.imaginaire.utils.config_helper import override
    ...
    if enable_one_logger:
        try:
            ...
            from gamma_world._src.imaginaire.utils.one_logger.one_logger_override_utils import override_one_logger_callback
...
def _load_py_config(config_path: str, opts: list[str], validate: bool = True) -> Config:
    ...
    from gamma_world._src.imaginaire.utils.config_helper import get_config_module, override
