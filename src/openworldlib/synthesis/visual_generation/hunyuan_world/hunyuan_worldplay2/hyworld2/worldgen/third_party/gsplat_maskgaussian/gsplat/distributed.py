...
def cli(fn: Callable, args: Any, verbose: bool = False) -> bool:
    ...
    print(f"[DDP-DEBUG] cli() entered. PID={os.getpid()}", flush=True)
    ...
    if "OMPI_COMM_WORLD_SIZE" in os.environ:  # multi-node
        ...
        print(
            f"[DDP-DEBUG] Multi-node mode: rank={world_rank}/{world_size} "
    ...
    distributed = world_size > 1
    print(
        f"[DDP-DEBUG] CUDA devices: {world_size}, distributed={distributed}",
        flush=True,
    )
    ...
    if distributed:
        ...
        print(
            f"[DDP-DEBUG] MASTER_ADDR=localhost MASTER_PORT={free_port}",
            flush=True,
        )
        print(
            f"[DDP-DEBUG] Spawning {world_size} workers via torch.multiprocessing.spawn ...",
            flush=True,
        )
        _t0 = _time.time()
        ...
        print(
            f"[DDP-DEBUG] spawn() returned in {_time.time() - _t0:.2f}s, "
            f"waiting for workers to join ...",
            flush=True,
        )
        try:
            process_context.join()
            print("[DDP-DEBUG] All workers joined successfully.", flush=True)
        except KeyboardInterrupt:
            print("[DDP-DEBUG] KeyboardInterrupt caught, terminating workers ...", flush=True)
            # this is important.
            ...
            for i, process in enumerate(process_context.processes):
                if process.is_alive():
                    if verbose:
                        print("terminating process " + str(i) + "...", flush=True)
                    process.terminate()
                process.join()
                if verbose:
                    print("process " + str(i) + " finished", flush=True)
        return True
    else:
        print("[DDP-DEBUG] Single GPU mode, running worker directly.", flush=True)
