...
__global__ void rasterize_to_pixels_3dgs_bwd_kernel(
    ...
    const scalar_t *__restrict__ gauss_masks, // [..., N] or [nnz], per-Gaussian mask (MaskGaussian)
    ...
    scalar_t *__restrict__ v_gauss_masks // [..., N] or [nnz], grad for per-Gaussian mask (MaskGaussian)
) {
    ...
    float *gauss_mask_batch =
        (float *)&rgbs_batch[block_size * CDIM]; // [block_size]
...
#pragma unroll
            ...
            // MaskGaussian: load per-Gaussian mask (1.0 if not provided)
            gauss_mask_batch[tr] = gauss_masks != nullptr ? (float)gauss_masks[g] : 1.0f;
        }
        ...
        for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size;
            ...
            float v_gauss_mask_local = 0.f;
            // initialize everything to 0, only set if the lane is valid
            if (valid) {
                // MaskGaussian: read per-Gaussian mask
                const float gm = gauss_mask_batch[t];
                const float ma = gm * alpha;  // effective alpha = mask * alpha

                // compute the current T for this gaussian
                // In forward: T_next = T * (1 - ma), so T_prev = T_next / (1 - ma)
                float ra = 1.0f / (1.0f - ma);
                T *= ra;
                // update v_rgb for this gaussian
                // C_i = color_i * ma * T, so dL/dcolor = ma * T * v_render_c
                const float fac = ma * T;
#pragma unroll
                for (uint32_t k = 0; k < CDIM; ++k) {
                    v_rgb_local[k] = fac * v_render_c[k];
                }
                // Compute dL/d(ma) — the gradient w.r.t. the effective alpha (mask*alpha)
                // This is the same formula as original v_alpha, but now for ma = mask*alpha
                float v_ma = 0.f;
#pragma unroll
                for (uint32_t k = 0; k < CDIM; ++k) {
                    v_ma += (rgbs_batch[t * CDIM + k] * T - buffer[k] * ra) *
                               v_render_c[k];
                }

                v_ma += T_final * ra * v_render_a;
...
#pragma unroll
                    ...
                    v_ma += -T_final * ra * accum;
                }

                // MaskGaussian: decompose dL/d(ma) into dL/d(alpha) and dL/d(mask)
                // Since ma = mask * alpha:
                //   dL/d(alpha) = dL/d(ma) * mask
                //   dL/d(mask)  = dL/d(ma) * alpha
                // Key insight: when mask=0, dL/d(alpha)=0 (opacity gets no gradient)
                // but dL/d(mask) = v_ma * alpha ≠ 0 (mask STILL gets gradient!)
                const float v_alpha = v_ma * gm;
                v_gauss_mask_local = v_ma * alpha;
...
#pragma unroll
            ...
            warpSum(v_gauss_mask_local, warp);
...
#pragma unroll
                ...
                // MaskGaussian: write gradient for per-Gaussian mask
                if (v_gauss_masks != nullptr) {
                    gpuAtomicAdd(v_gauss_masks + g, v_gauss_mask_local);
                }
            }
        }
    }
...
void launch_rasterize_to_pixels_3dgs_bwd_kernel(
    ...
    const at::optional<at::Tensor> gauss_masks, // [..., N] or [nnz], per-Gaussian mask (MaskGaussian)
    // image size
    const uint32_t image_width,
    const uint32_t image_height,
    const uint32_t tile_size,
    ...
    at::optional<at::Tensor> v_gauss_masks  // [..., N] or [nnz], grad for per-Gaussian mask
) {
    ...
    int64_t shmem_size =
        tile_size * tile_size *
        (sizeof(int32_t) + sizeof(vec3) + sizeof(vec3) + sizeof(float) * CDIM + sizeof(float));
    ...
    rasterize_to_pixels_3dgs_bwd_kernel<CDIM, float>
        <<<grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
            ...
            gauss_masks.has_value() ? gauss_masks.value().data_ptr<float>()
                                    : nullptr,
            image_width,
            image_height,
            tile_size,
            ...
            v_gauss_masks.has_value() ? v_gauss_masks.value().data_ptr<float>()
                                      : nullptr
        );
}
...
#define __INS__(CDIM)                                                          \
    template void launch_rasterize_to_pixels_3dgs_bwd_kernel<CDIM>(            \
        ...
        const at::optional<at::Tensor> gauss_masks,                            \
        ...
        at::optional<at::Tensor> v_gauss_masks                                 \
    );
