...
std::tuple<at::Tensor, at::Tensor, at::Tensor> rasterize_to_pixels_3dgs_fwd(
    ...
    if (gauss_masks.has_value()) {
        CHECK_INPUT(gauss_masks.value());
    }
...
rasterize_to_pixels_3dgs_bwd(
    ...
    // MaskGaussian: allocate gradient tensor for per-Gaussian mask
    at::Tensor v_gauss_masks_out;
    if (gauss_masks.has_value()) {
        v_gauss_masks_out = at::zeros_like(gauss_masks.value());
    }
...
#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_3dgs_bwd_kernel<N>(                         \
            ...
            v_colors,                                                          \
            v_opacities,                                                       \
            gauss_masks.has_value() ? c10::optional<at::Tensor>(v_gauss_masks_out) : c10::nullopt \
        );                                                                     \
        break;
...
#undef __LAUNCH_KERNEL__
    ...
    return std::make_tuple(
        v_means2d_abs, v_means2d, v_conics, v_colors, v_opacities, v_gauss_masks_out
    );
}
