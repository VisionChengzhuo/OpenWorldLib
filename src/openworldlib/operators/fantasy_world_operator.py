...
class FantasyWorldOperator(BaseOperator):
    def __init__(self, operation_types=None, interaction_template=None) -> None:
        if operation_types is None:
            operation_types = ["visual_instruction", "textual_instruction"]
    ...
    def check_interaction(self, interaction: str) -> bool:
        if not isinstance(interaction, str):
            raise TypeError(f"FantasyWorld prompt must be a string, got {type(interaction)}")
    ...
    def process_perception(
        ...
        camera_json_path: Union[str, Path],
    ) -> Dict[str, str]:
        return {
