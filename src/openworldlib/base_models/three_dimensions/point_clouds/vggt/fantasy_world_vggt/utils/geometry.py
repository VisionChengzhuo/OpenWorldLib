...
from ..dependency.distortion import *


def unproject_depth_map_to_point_map(
        depth_map: np.ndarray,
        extrinsics_cam: np.ndarray,
        intrinsics_cam: np.ndarray) -> np.ndarray:
    """
...
def depth_to_world_coords_points(
    ...
    # extrinsic_inv is 4x4 (note closed_form_inverse_OpenCV is batched, the
    # output is (N, 4, 4))
    ...
    world_coords_points = np.dot(
        cam_coords_points,
        R_cam_to_world.T) + t_cam_to_world  # HxWx3, 3x3 -> HxWx3

    return world_coords_points, cam_coords_points, point_mask


def depth_to_cam_coords_points(
        depth_map: np.ndarray, intrinsic: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    assert intrinsic[0, 1] == 0 and intrinsic[1,
                                              0] == 0, "Intrinsic matrix must have zero skew"
...
def project_world_points_to_camera_points_batch(world_points, cam_extrinsics):
    ...
    camera_points = torch.matmul(
        extrinsics_exp, world_points_h_exp).squeeze(-1)
...
def project_world_points_to_cam(
    ...
    with torch.autocast(device_type=device.type, enabled=False):
        ...
        world_points_homogeneous = world_points_homogeneous.unsqueeze(
            0).expand(B, -1, -1)  # BxNx4
