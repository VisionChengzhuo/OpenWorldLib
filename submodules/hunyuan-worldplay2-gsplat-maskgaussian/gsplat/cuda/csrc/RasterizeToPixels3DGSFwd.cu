...
__global__ void rasterize_to_pixels_3dgs_fwd_kernel(
    ...
    const scalar_t *__restrict__ gauss_masks, // [I, N] or [nnz], per-Gaussian mask (MaskGaussian)
...
#pragma unroll
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            // MaskGaussian: alpha is computed WITHOUT mask.
            // The alpha < ALPHA_THRESHOLD check does NOT involve mask,
            // so masked-out Gaussians still pass through if they overlap the pixel.
            float alpha = min(0.999f, opac * __expf(-sigma));
            if (sigma < 0.f || alpha < ALPHA_THRESHOLD) {
                continue;
            }

            // MaskGaussian: use mask*alpha for transmittance update
            const float gm = gauss_mask_batch[t];
            const float next_T = T * (1.0f - gm * alpha);
            ...
            // MaskGaussian: vis = mask * alpha * T
            const float vis = gm * alpha * T;
...
void launch_rasterize_to_pixels_3dgs_fwd_kernel(
    ...
    int64_t shmem_size =
        tile_size * tile_size * (sizeof(int32_t) + sizeof(vec3) + sizeof(vec3) + sizeof(float));
